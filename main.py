...
transformer_file_path = PREPROCESSOR_OBJ_FILE
...
def perform_batch_prediction():
    ...
    if request.method == "GET":
        ...
        #check if the file has a valid extension(csv extension) or not
        if file and '.' in file.filename.rsplit('.',1)[1].lower() in ALLOW_EXTENSION:
